...
function Entrevista({ passo, setPasso, r, setR, fim, cancelar, extraQuestions }: { passo: number; setPasso: (n: number) => void; r: Record<string, string>; setR: RespostasSetter; fim: () => void; cancelar: () => void; extraQuestions: SurveyQuestion[] }) {
  ...
  return <div className="entrevista"><div className="entrevista-topo"><div><small>BETIM · TERRITÓRIO E ESCOLHAS 2026</small><h2>Entrevista <span>ENT-2026-000799</span></h2></div><label>✓ Respostas salvas neste dispositivo</label></div><div className="passos">{["Consentimento", "Território", "Gestão", "Lideranças", "6 escolhas", "Perfil", "Contato", ...(visibleExtras.length ? ["Adicionais"] : [])].map((x, i) => <div className={i + 1 <= passo ? "feito" : ""} key={x}><i>{i + 1 < passo ? "✓" : i + 1}</i><span>{x}</span></div>)}</div><div className="questao"><small>ETAPA {passo} DE {totalPassos} · COLETA EM CAMPO</small>
    ...
    {passo === 5 && <><h3>Intenção de voto</h3><p className="instrucao">Registre somente o que a pessoa disser. Não apresente nomes nem sugira respostas. Se não houver nome, escreva: Não sabe, Branco/nulo, Não pretende votar ou Prefere não responder.</p><label>1. Se a eleição para deputado federal fosse hoje, em quem você votaria? *<input value={r.votoFederal || ""} onChange={e => set("votoFederal", e.target.value)} placeholder="Resposta espontânea" autoComplete="off" /></label><label>2. E para deputado estadual, em quem você votaria? *<input value={r.votoEstadual || ""} onChange={e => set("votoEstadual", e.target.value)} placeholder="Resposta espontânea" autoComplete="off" /></label><label>3. Para senador, qual seria sua primeira escolha? *<input value={r.votoSenador1 || ""} onChange={e => set("votoSenador1", e.target.value)} placeholder="Resposta espontânea" autoComplete="off" /></label><label>4. E qual seria sua segunda escolha para senador? *<input value={r.votoSenador2 || ""} onChange={e => set("votoSenador2", e.target.value)} placeholder="Resposta espontânea" autoComplete="off" /></label><label>5. Para governador de Minas Gerais, em quem você votaria? *<input value={r.votoGovernador || ""} onChange={e => set("votoGovernador", e.target.value)} placeholder="Resposta espontânea" autoComplete="off" /></label><label>6. Para presidente da República, em quem você votaria? *<input value={r.votoPresidente || ""} onChange={e => set("votoPresidente", e.target.value)} placeholder="Resposta espontânea" autoComplete="off" /></label><label>Hoje, sua decisão de voto está?</label><Opcoes campo="certezaVoto" itens={["Totalmente decidida", "Pode mudar", "Ainda não decidiu", "Prefere não responder"]} /></>}
