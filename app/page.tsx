  ...
  useEffect(() => {
    ...
    const checkEvents = async () => {
      try {
        ...
        if (events.length > known) aviso(`${events.length - known} nova(s) ocorrência(s) de campo: confira recusas e interrupções.`);
        known = events.length;
        setFieldEvents(events);
...
function Entrevista({ passo, setPasso, r, setR, fim, cancelar, extraQuestions }: { passo: number; setPasso: (n: number) => void; r: Record<string, string>; setR: RespostasSetter; fim: () => void; cancelar: () => void; extraQuestions: SurveyQuestion[] }) {
  ...
  const RespostaEleitoral = ({ campo, titulo, ajuda }: { campo: string; titulo: string; ajuda?: string }) => <div className="resposta-eleitoral"><label htmlFor={campo}>{titulo} *</label>{ajuda && <small>{ajuda}</small>}<input id={campo} value={r[campo] || ""} onChange={e => set(campo, e.target.value)} placeholder="Digite o nome citado ou 'Não sabe'" autoComplete="off" /><p>Se não houver nome, escreva: Não sabe, Branco/nulo, Não pretende votar ou Prefere não responder.</p></div>;
  ...
  return <div className="entrevista"><div className="entrevista-topo"><div><small>BETIM · TERRITÓRIO E ESCOLHAS 2026</small><h2>Entrevista <span>ENT-2026-000799</span></h2></div><label>✓ Respostas salvas neste dispositivo</label></div><div className="passos">{["Consentimento", "Território", "Gestão", "Lideranças", "6 escolhas", "Perfil", "Contato", ...(visibleExtras.length ? ["Adicionais"] : [])].map((x, i) => <div className={i + 1 <= passo ? "feito" : ""} key={x}><i>{i + 1 < passo ? "✓" : i + 1}</i><span>{x}</span></div>)}</div><div className="questao"><small>ETAPA {passo} DE {totalPassos} · COLETA EM CAMPO</small>
    ...
    {passo === 5 && <><h3>Intenção de voto</h3><p className="instrucao">Registre somente o que a pessoa disser. Não apresente nomes, não sugira opções e não use botões nesta etapa.</p><RespostaEleitoral campo="votoFederal" titulo="1. Se a eleição para deputado federal fosse hoje, em quem a pessoa votaria?" /><RespostaEleitoral campo="votoEstadual" titulo="2. E para deputado estadual, em quem a pessoa votaria?" /><RespostaEleitoral campo="votoSenador1" titulo="3. Para senador, qual seria a primeira escolha?" /><RespostaEleitoral campo="votoSenador2" titulo="4. E qual seria a segunda escolha para senador?" ajuda="Registre uma resposta diferente da primeira ou uma opção de não resposta." /><RespostaEleitoral campo="votoGovernador" titulo="5. Para governador de Minas Gerais, em quem a pessoa votaria?" /><RespostaEleitoral campo="votoPresidente" titulo="6. Para presidente da República, em quem a pessoa votaria?" /><label>Hoje, a decisão de voto está?</label><Opcoes campo="certezaVoto" itens={["Totalmente decidida", "Pode mudar", "Ainda não decidiu", "Prefere não responder"]} /></>}
