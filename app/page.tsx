    ...
    <main>
      <header>
        ...
        <div className={campo ? "marca-campo" : ""}>
          <small>{campo ? "NORTEP PESQUISA · ÁREA DO PESQUISADOR · V47" : "NORTEP · DADOS QUE APROXIMAM · V47"}</small>
          <h1>{titulos[view]}</h1>
        </div>
        <section>
...
function Login({ access, inviteCode, onAuthenticated }: { access: AccessChannel; inviteCode: string; onAuthenticated: (session: Session, channel?: AccessChannel) => Promise<void> }) {
  ...
  return <div className="auth-shell"><ControleFonte />
    ...
    <form className="auth-card" onSubmit={e => { e.preventDefault(); void enviar(); }}>
      ...
      <small className="auth-help">{adminAccess || coordinatorAccess || supervisorAccess || observerAccess ? `Este link é exclusivo para ${accessName} autorizada.` : "O entrevistado não precisa criar conta."} · V47</small>
    </form>
  </div>;
}
...
function ObserverPanel({ profile, summary, sair, descadastrar, atualizar }: { profile: Profile; summary: ObserverSummary | null; sair: () => void; descadastrar: () => Promise<void>; atualizar: () => Promise<void> }) {
  ...
  return <div className="observer-shell"><ControleFonte profile={profile} sair={sair} descadastrar={descadastrar} />
    <aside><div className="observer-logo"><i>NP</i><span>NorteP <b>Pesquisa</b></span></div><div className="observer-lock"><b>◉ Painel geral da coleta</b><span>Indicadores consolidados das pesquisas em andamento.</span></div><div className="observer-user"><i>{profile.name.split(" ").slice(0, 2).map(x => x[0]).join("").toUpperCase()}</i><span><b>{profile.name}</b><small>Observador autorizado</small></span><button onClick={sair}>Sair</button></div></aside>
    <main><header><div><small>NORTEP · VISÃO AUTORIZADA</small><h1>Acompanhamento da coleta</h1></div><button onClick={refresh} disabled={busy}>{busy ? "Atualizando…" : "↻ Atualizar números"}</button></header><div className="observer-content"><div className="observer-intro"><div><small>VISÃO GERAL</small><h2>Olá, {profile.name.split(" ")[0]}.</h2><p>Acompanhe o andamento das pesquisas por meio dos indicadores consolidados da operação.</p></div><span>Última coleta: <b>{lastUpdate}</b></span></div>{summary ? <><div className="observer-metrics"><Metrica c="verde" i="✓" t="Entrevistas realizadas" v={String(summary.total_interviews)} s="total da operação" /><Metrica c="laranja" i="◎" t="Realizadas hoje" v={String(summary.interviews_today)} s="ritmo da coleta" /><Metrica c="roxo" i="♙" t="Pesquisadores com coleta" v={String(summary.active_researchers)} s="participação em campo" /><Metrica c="azul" i="▤" t="Pesquisas em andamento" v={String(summary.active_surveys)} s="pesquisas liberadas" /></div><div className="painel observer-surveys"><div className="topo"><div><small>PESQUISAS EM ANDAMENTO</small><h3>Acompanhamento geral</h3></div></div>{summary.surveys.length ? summary.surveys.map(item => <div className="observer-survey-row" key={item.id}><span><b>{item.title}</b><small>Em campo</small></span><div><b>{item.interviews}</b><small>entrevistas</small></div><div><b>{item.researchers}</b><small>pesquisadores</small></div></div>) : <div className="resultado-vazio"><h3>Nenhuma pesquisa em andamento</h3><p>Os indicadores aparecerão quando uma pesquisa for liberada.</p></div>}</div></> : <div className="painel resultado-vazio"><h3>Preparando indicadores</h3><p>Aguarde um momento e atualize os números.</p></div>}<div className="observer-notice"><i>✓</i><span><b>Leitura dos indicadores</b><small>Os números mostram volume, ritmo e participação por pesquisa de forma agrupada.</small></span></div></div></main>
  </div>;
}
