...
const pesquisas = [
  { nome: "Betim: território e escolhas 2026", status: "Liberada", feitas: 0, meta: 100, equipe: 5 },
  { nome: "Avaliação dos serviços públicos", status: "Planejada", feitas: 0, meta: 500, equipe: 0 },
  { nome: "Prioridades da comunidade", status: "Planejada", feitas: 0, meta: 400, equipe: 0 },
];
...
function TelaConfigErro() {
  return <div className="auth-shell"><div className="auth-card"><div className="auth-logo">NP</div><h1>Acesso temporariamente indisponível</h1><p>Tente novamente em alguns minutos. Se o problema continuar, fale com a equipe NorteP.</p></div></div>;
}
...
function Pesquisas({ ir, aviso, videoUrl, setVideoUrl }: { ir: (v: View) => void; aviso: (t: string) => void; videoUrl: string; setVideoUrl: (v: string) => void }) {
  ...
  return <>
    ...
    <div className="cards">{pesquisas.map((p, i) => <article key={p.nome}><div><label className={p.status === "Liberada" ? "status" : "rascunho"}>{p.status}</label><button>•••</button></div><h3>{p.nome}</h3><p>{i === 0 ? "Diagnóstico territorial, serviços públicos, lideranças e intenção de voto nas seis escolhas de 2026." : i === 1 ? "Avaliação da saúde, educação, limpeza e transporte público." : "Mapeamento das principais demandas dos moradores."}</p>{i === 0 && <div className="video-status"><i>▶</i><span><b>Vídeo de agradecimento</b><small>{videoUrl ? "Link do YouTube configurado" : "Opcional · não configurado"}</small></span></div>}<section><span><b>{p.feitas}</b> respostas</span><span><b>{p.equipe}</b> pesquisadores</span><span><b>{i === 0 ? 39 : i === 2 ? 18 : 24}</b> perguntas</span></section><div className="progresso"><small>{p.feitas} de {p.meta}</small><em><i style={{ width: (p.feitas / p.meta * 100) + "%" }} /></em></div><footer>{i === 0 && <button onClick={configurarVideo}>▶ Vídeo</button>}<button onClick={() => aviso("Editor de perguntas aberto")}>Editar</button><button onClick={() => i === 0 ? ir("portal") : i === 2 ? aviso("Pesquisa liberada para a equipe") : ir("resultados")}>{i === 0 ? "Aplicar" : i === 2 ? "Liberar" : "Acompanhar"}</button></footer></article>)}</div>
  </>;
}
