...
function Entrevista({ passo, setPasso, r, setR, fim, cancelar, extraQuestions }: { passo: number; setPasso: (n: number) => void; r: Record<string, string>; setR: RespostasSetter; fim: () => void; cancelar: () => void; extraQuestions: SurveyQuestion[] }) {
  ...
  const Intencao = ({ campo, titulo, ajuda }: { campo: string; titulo: string; ajuda?: string }) => {
    const semNome = r[`${campo}SemNome`] || "";
    return <div className="intencao"><label htmlFor={campo}>{titulo} *</label>{ajuda && <small>{ajuda}</small>}<input id={campo} value={r[campo] || ""} onChange={e => { set(campo, e.target.value); if (e.target.value) set(`${campo}SemNome`, ""); }} placeholder="Digite exatamente o nome citado" autoComplete="off" /><label className="resposta-sem-nome">Se não foi citado um nome<select value={semNome} onChange={e => { set(`${campo}SemNome`, e.target.value); if (e.target.value) set(campo, ""); }}><option value="">Selecione uma opção</option><option value="Não sabe">Não sabe</option><option value="Branco/nulo">Branco ou nulo</option><option value="Não pretende votar">Não pretende votar</option><option value="Prefere não responder">Prefere não responder</option></select></label></div>;
  };
  ...
  const obrigatorios: Record<number, boolean> = {
    ...
    5: ["votoFederal", "votoEstadual", "votoSenador1", "votoSenador2", "votoGovernador", "votoPresidente"].every(k => textoValido(r[k], 2) || Boolean(r[`${k}SemNome`])),
