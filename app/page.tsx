    ...
    <main>
      <header>
        ...
        <div className={campo ? "marca-campo" : ""}>
          <small>{campo ? "NORTEP PESQUISA · ÁREA DO PESQUISADOR · V34" : "NORTEP · DADOS QUE APROXIMAM · V34"}</small>
          <h1>{titulos[view]}</h1>
        </div>
        <section>
...
function Login({ access, inviteCode, onAuthenticated }: { access: AccessChannel; inviteCode: string; onAuthenticated: (session: Session, channel?: AccessChannel) => Promise<void> }) {
  ...
  return <div className="auth-shell"><ControleFonte />
    ...
    <form className="auth-card" onSubmit={e => { e.preventDefault(); void enviar(); }}>
      ...
      <small className="auth-help">{adminAccess || coordinatorAccess || observerAccess ? `Este link é exclusivo para ${accessName} autorizada.` : "O entrevistado não precisa criar conta."} · V34</small>
    </form>
  </div>;
}
